    ...
    for(var i = 0; i < nDiscretizationPointsCount; ++i)
    {
        for(var j = 0; j<eDiscretizationPointsCount; ++j)
        {
            ...
            if(i == nDiscretizationPointsCount - 1)
            {
                ...
                /*console.log("Result cp ["+(i * eDiscretizationPointsCount + j)+"] = "+controlPoints[controlPointsRowsCount - 1][0].x
                                             +", "+controlPoints[controlPointsRowsCount - 1][0].y
                                             +", "+controlPoints[controlPointsRowsCount - 1][0].z);*/
            }
            else
            {
                ...
                //console.log("Result tmp ["+(i * eDiscretizationPointsCount + j)+"] = "+(x_tmp / tmp)+", "+(y_tmp / tmp)+", "+(z_tmp / tmp));
               // console.log("tmp "+tmp+", x_tmp "+x_tmp+", y_tmp "+y_tmp+", z_tmp "+z_tmp);
            }
        } // eol
    } // eol
...
function CalculateArrayOfKnots(controlPointsCount, basisFunctionOrder, knotVectorType) // [x]
{
    ...
    else
    {
        ...
        //console.log("Not periodic (propably open)");
    }
    return knotVector;
}
